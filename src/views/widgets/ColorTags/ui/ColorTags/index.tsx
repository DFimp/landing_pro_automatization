...
export default function ColorTags({ searchParams }: WidgetPageProps) {
  return (
    <WidgetPageTemplate
      widget={widget}
      ...
      preInstruction={
        <>
          <WidgetHeroContent
            badge={<WidgetHeroBadge>Раскраска тегов для amoCRM</WidgetHeroBadge>}
            title="Выделяйте важные сделки цветом тегов"
            description="Виджет автоматически раскрашивает выбранные теги в списке сделок, на доске канбан и внутри карточки сделки. Команда быстрее замечает приоритетные сделки и не теряет фокус."
          ...
          <FeaturesCard
            title="Что дает раскраска тегов?"
            subtitle="Быстрая визуальная навигация по приоритетам прямо в воронке"
            items={FEATURES}
          />

          <ProblemsCard
            title="Когда виджет особенно полезен?"
            subtitle="Типовые сценарии, где цветные теги экономят время менеджеров"
            items={PROBLEMS}
          />
        </>
      }
      instruction={
        <SettingsCard
          ...
          subtitle="Подключите виджет, выберите теги и задайте цвета для приоритетных сделок:"
          steps={STEPS}
          showVideo={false}
        />
      }
